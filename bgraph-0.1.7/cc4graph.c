...
static void
setPx(word x, byte y, byte color)
{
	task1();
	Primitive_setPixel320x16(x, y, color, GRAPHICS_START);
	task0();
}

static void
getPx(word x, byte y, byte* color)
{
	task1();
	*color = Primitive_getPixel320x16(x, y, GRAPHICS_START);
	task0();
}
